...
class ItemInstController extends CI_Controller {
		...
		date_default_timezone_set('Asia/Kolkata');         
            ...
            //  $this->load->library('Signup_controller');
             $loginstatus   = $this->session->userdata('logged_in'); 
              $this->load->model('MenuSelection/ItemInstModel','obj');
...
//                        ['error'=>$error['error'], 'flag'=>0, 'dataItem'=>$dataItem,'serviceData'=>$service, 'category'=>$dataItemCategory]);
             //   }
              ...
              //  {
                       ...
                       //echo str_replace(' ', '_', $fname);
                        ...
                        if($this->obj->insertItem($item, $categoryID, $price, $imagePath))
                        {
                           redirect('iteminst', 'refresh');
                        }
    ...
    public function editItem()
    {
                   ...
                   else
                   {
                       ...
                       else 
                       {
                           if (move_uploaded_file($_FILES["userfile"]["tmp_name"], $target_file)) 
                           {
                               ...
                               $target_file1 = $fname;
                               $data = $this->obj->updateItemByID($ItemID, $itemName,$price, $categoryid, $target_file1);
                               if($data)
                               {
                                   redirect('iteminst/index', 'refresh');
                               }
                           } 
                           else 
                           {
        ...
        else
        {
           ...
           $this->load->view('Admin/MenuCreation/editItemView',
